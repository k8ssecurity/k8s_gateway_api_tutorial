...
1. **AgentgatewayBackend `openai-llm-backend`** — `spec.ai.provider.openai` (upstream defaults to `api.openai.com:443`, no SNI needed), pinned to `gpt-4o`, with `policies.auth.secretRef` pointing at `openai-secret`.
